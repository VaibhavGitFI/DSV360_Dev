...
async function importBulkData (environment, catalystApp, tableName, query, orgId, workspaceId, viewId, callbackUrl) {
  const credentials = {
    bulk_connector: {
      client_id: process.env[AppConstants.Env.Projects_ClientId],
      client_secret: process.env[AppConstants.Env.Projects_ClientSecret],
      auth_url: process.env[AppConstants.Env.Projects_AuthHost],
      refresh_url: process.env[AppConstants.Env.Projects_AuthHost],
      refresh_token: process.env[AppConstants.Env.Projects_RefreshToken]
    }
  }
