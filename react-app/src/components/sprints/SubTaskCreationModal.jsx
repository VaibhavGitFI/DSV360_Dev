...
const SubTaskCreationModal = ({
  ...
  }, [open, onClose, TaskStatus]);

  if (!open || !story || !task) return null;
  ...
  const submit = (e) => {
    ...
    onSubmit?.({
      ...
      status: form.status || (TaskStatus?.TODO || "TODO"),
      estimatedHours: (Number(form.estHours) || 0) + (Number(form.estMinutes) || 0) / 60,
    ...
    >
      ...
      >
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 800, color: C.text, opacity: 0.75 }}>
              For: {story.displayId} → {task.displayId} — {task.title}
            </div>
          </div>
        ...
        <form onSubmit={submit} style={{ padding: 18, display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            <div style={label}>Sub-task title</div>
            <input
              autoFocus
              value={form.title}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div>
              ...
              <select value={form.status} onChange={(e) => setForm((p) => ({ ...p, status: e.target.value }))} style={selectBase}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <div style={{ flex: 1 }}>
                  <input
                    ...
                    max={999}
                    value={form.estHours}
                    onChange={(e) => setForm((p) => ({ ...p, estHours: e.target.value }))}
                    style={inputBase}
                    placeholder="0"
                  />
                  <div style={{ fontSize: 9, fontWeight: 800, color: C.muted, marginTop: 4, textAlign: "center", textTransform: "uppercase", letterSpacing: "0.1em" }}>Hours</div>
                </div>
                <div style={{ flex: 1 }}>
                  <input
                    type="number"
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, estMinutes: Math.min(59, Math.max(0, Number(e.target.value) || 0)) }))}
                    style={inputBase}
                    placeholder="0"
                  />
                  <div style={{ fontSize: 9, fontWeight: 800, color: C.muted, marginTop: 4, textAlign: "center", textTransform: "uppercase", letterSpacing: "0.1em" }}>Minutes</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div>
              ...
              <select value={form.assigneeId} onChange={(e) => setForm((p) => ({ ...p, assigneeId: e.target.value }))} style={selectBase}>
