...
import TimeEntryApproval from "../Employee/TimeEntryApproval";
...
export const TimeEntry = ({
  ...
  // console.log("Employee data in time",employeeData);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // console.log("empoliyee lengthhtht",employeeData.length);
        ...
        // Apply filter
        // if (filterType === "Billable") {
        //   entries = entries.filter((entry) => entry.details.Type === "Billable");
        // } else if (filterType === "non-billable") {
        //   entries = entries.filter((entry) => entry.isBillable === false);
        // }
  ...
  const handleAddTimesheetEntry = async () => {
   ...
   console.log("Adding timesheet entry:", viewTask);
  ...
  const handleUpdateTimeEntry = async () => {
    ...
    try {
      await axios.post(
        ...
        {
          Username: currentEditTimeEntry.name,
          User_ID: currentEditTimeEntry.userid,
  ...
  const handleEdit = (entry) => {
    try {
      ...
      setcurrentEditTimeEntry({
        ...
        userid:entry.Time_Entries.User_ID,
...
>
                  ...
                  <Grid container spacing={2}>
                    ...
                    {/* <Grid item xs={12}>
                      <TextField
                       ...
                       inputProps={{ spellCheck: true, autoCorrect: "on" }}
                      />
                    </Grid> */}
                    <Grid item xs={12}>
  <TextField
  ...
  >
    ...
    <FormControl size="small" sx={{ minWidth: 150 }}>
      ...
      >
        ...
        <MenuItem value="Non-Billable">Non-Billable</MenuItem>
                  ...
                  <Card sx={{ mt: 2, mb: 2, mx: 0 }}>
                    ...
                    >
                      {/* Left Side: Date Pickers & Submit Button */}
                      <Box
                        sx={{
                          display: "flex",
                          alignItems: "center",
                              ...
                              {viewTask &&
                                item.details?.map((entry, index) => (
                                  ...
                                  <TableRow key={index}>
                                    ...
                                    <TableCell>
                                      {new Date(entry.Time_Entries.Entry_Date).toLocaleDateString("en-GB")}
                                    </TableCell>
                                    <TableCell>
      ...
      userId={currUser.userid}
    reqOpen={reqOpen}
url = {`/server/time_entry_management_application_function/timeentry/approval?taskId=${viewTask.ROWID}`} 
